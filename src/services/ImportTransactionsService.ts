...
class ImportTransactionsService {
  async execute(filePath: string): Promise<Transaction[]> {
    ...
    const parsers = csvParse({ // creating parser from csv
      delimiter: ',',
    ...
    parseCSV.on('data', async line => {
      const [title, type, value, category] = line.map((cell: string) => cell.trim()); // pull of spaces on line
    ...
    const existentCategoriesTitles = existentCategories.map((category: Category) => category.title); // getting just the title
    const addCategories = categories.filter(category => !existentCategoriesTitles.includes(category))
